defmodule AMath.Game.Rule do
  ...
  def get_linear([%{i: x0, j: y0} | rest] = points) when is_list(points) do
    cond do
      ...
      Enum.all?(rest, fn %{i: xn} -> xn == x0 end) ->
        {%{j: min_y}, %{j: max_y}} = points |> Enum.min_max_by(fn %{j: y} -> y end)

        {:constant_x, x0, min_y, max_y}

      Enum.all?(rest, fn %{j: yn} -> yn == y0 end) ->
        {%{i: min_x}, %{i: max_x}} = points |> Enum.min_max_by(fn %{i: x} -> x end)
  ...
  def is_equation_correct(items) do
    ...
    with true <- Enum.member?(items, "="),
         ...
         :ok <- Macro.validate(ast) do
      all_expressions_matched?(equations)
    else
      _ -> false
    end
  end

  defp all_expressions_matched?(list) do
    try do
      # Poor algorithm for now
      count =
        list
        |> Enum.join()
        |> String.split("=")
        |> Enum.map(fn expr -> Code.eval_string(expr) |> elem(0) |> Kernel./(1) end)
        |> Enum.uniq()
        |> Enum.count()
